...
--Bang PhieuThuTien
ALTER TABLE PHIEUTHUTIEN ADD CONSTRAINT FK_PHIEUTHUTIEN_PHIEUXUATHANG FOREIGN KEY(MaPhieuXuat) REFERENCES PHIEUXUATHANG(MaPhieuXuat);
...
/
/*TRIGGER*/
/*Khi insert, update mot CHITIETPHIEUNHAP thi Tong tien nhap cung thay doi*/
SET DEFINE OFF;
CREATE OR REPLACE TRIGGER TRIGGER_INSERT_UPDATE_TIENNHAP_CTPN
AFTER INSERT OR UPDATE ON CHITIETPHIEUNHAP 
FOR EACH ROW
DECLARE
BEGIN
    UPDATE PHIEUNHAPHANG
    SET TONGTIEN = TONGTIEN + (:NEW.SOLUONG * :NEW.DONGIANHAP)
    WHERE MAPHIEUNHAP = :NEW.MAPHIEUNHAP;
END;
/
/*Khi delete, update mot CHITIETPHIEUNHAP thi Tong tien nhap cung thay doi*/
SET DEFINE OFF;
CREATE OR REPLACE TRIGGER TRIGGER_DELETE_UPDATE_TIENNHAP_CTPN
AFTER DELETE OR UPDATE ON CHITIETPHIEUNHAP 
FOR EACH ROW
DECLARE
BEGIN
    UPDATE PHIEUNHAPHANG
    SET TONGTIEN = TONGTIEN - (:OLD.SOLUONG * :OLD.DONGIANHAP)
    WHERE MAPHIEUNHAP = :OLD.MAPHIEUNHAP;
END;
/
/*Khi insert, update mot CHITIETPHIEUXUAT thi Tong tien xuat cung thay doi*/
 --DROP TRIGGER TRIGGER_INSERT_UPDATE_TIENXUAT_CTPX;
SET DEFINE OFF;
CREATE OR REPLACE TRIGGER TRIGGER_INSERT_UPDATE_TIENXUAT_CTPX
AFTER INSERT OR UPDATE ON CHITIETPHIEUXUAT
FOR EACH ROW
DECLARE
BEGIN
    UPDATE PHIEUXUATHANG
    SET TONGTIEN = TONGTIEN + (:NEW.SOLUONG * :NEW.DONGIAXUAT)
    WHERE MAPHIEUXUAT = :NEW.MAPHIEUXUAT;
END;
/
/*Khi delete, update mot CHITIETPHIEUXUAT thi Tong tien xuat cung thay doi*/
SET DEFINE OFF;
CREATE OR REPLACE TRIGGER TRIGGER_DELETE_UPDATE_TIENXUAT_CTPX
AFTER DELETE OR UPDATE ON CHITIETPHIEUXUAT 
FOR EACH ROW
DECLARE
BEGIN
    UPDATE PHIEUXUATHANG
    SET TONGTIEN = TONGTIEN - (:OLD.SOLUONG * :OLD.DONGIAXUAT)
    WHERE MAPHIEUXUAT = :OLD.MAPHIEUXUAT;
END;
/
/*Don gia nhap va don gia xuat cua mat hang bang voi lan nhap hang cuoi cung*/
SET DEFINE OFF;
CREATE OR REPLACE TRIGGER TRIGGER_UPDATE_DONGIANHAP_DONGIAXUAT_MATHANG
...
BEGIN
    ...
    UPDATE MATHANG
    SET DONGIAXUAT= :NEW.DONGIANHAP*1.02
    WHERE MAMATHANG = :NEW.MAMATHANG;
END;
/
--Test
INSERT INTO CHITIETPHIEUNHAP VALUES('CTPN07', 'MH01', 'PN003', 100, 19500, 1950000);
