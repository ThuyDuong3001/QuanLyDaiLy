    	...
    	if (evt.getSource() == jButton3) {
    		...
    		try (Connection conn = ConnectionUtils.getMyConnection()) 
    		{
                if (conn != null) {
                    ...
                    String query = "select * from daily where MADAILY = \'" + query_text +"\' or maquan = \'" + query_text +"\' or maloaidaily = \'" + query_text + "\'";
	...
	private void jButton6ActionPerformed(java.awt.event.ActionEvent evt) {                                         
    	...
    	if (evt.getSource() == jButton6) {
    		...
    		if (row_select >= 0) {
                ...
                try {  
                    long a = Long.parseLong(jTextField9.getText());  
    			...
    			try (Connection conn = ConnectionUtils.getMyConnection()) 
    			{
    	            if (conn != null) {
            			...
            			Statement stsltd = conn.createStatement();
            			ResultSet rsdltd = stsltd.executeQuery("select * from thamso");
            			int dltd = 0;
            			while (rsdltd.next()){
            				dltd = rsdltd.getInt("sodailytoida");
            			}
            			
            			Statement stcountq = conn.createStatement();
            			ResultSet rscountq = stcountq.executeQuery("select * from daily");
            			int countq = 0;
            			while (rscountq.next()){
            				if (quan.equals(rscountq.getString("maquan"))){
            					countq += 1;
            				}
            			}
            			
            			if (countq >= dltd & !quan.equals(queries[row_select][3])) {
                        	JOptionPane.showMessageDialog(null,
